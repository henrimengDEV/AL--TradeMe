...
import org.esgi.core.member.domain.MemberBuilder;
...
public class CreateMemberHandler implements CommandHandler<CreateMember, MemberId> {
  ...
  public CreateMemberHandler(MemberRepository memberRepository, EventDispatcher<DomainEvent> domainEventDispatcher) {
  ...
  public MemberId handle(CreateMember command) {
    ...
    final List<JobType> competencesList = command.competences.stream().map(JobType::fromString)
    ...
    final Member member = MemberBuilder.builder().withFirstName(command.firstname).withLastName(command.lastname)
                                       .withLogin(command.login).withPassword(command.password).withMemberId(memberId)
                                       .withMemberRole(command.memberRole).withAddress(
            AddressFactory.create(command.address.city, command.address.country, command.address.street,
                                  command.address.zipCode)).withMail(command.mail).withGeographicZoneOfAvailability(
            GeographicZone.fromString(command.geographicZoneOfAvailability)).withCompetences(competencesList).build();
    Member memberAdded = memberRepository.add(member);

    domainEventDispatcher.dispatch(
        MemberCreatedEvent.create().withLogin(memberAdded.getLogin()).withMail(memberAdded.getMail()));
