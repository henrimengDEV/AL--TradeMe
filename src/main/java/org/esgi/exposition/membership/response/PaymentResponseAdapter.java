...
public class PaymentResponseAdapter implements Adapter<Payment, PaymentResponse> {
  ...
  public PaymentResponse adapt(Payment source) {
    return new PaymentResponse(source.getTransactionId().getValue(), source.getMemberId().getValue(),
                               source.getPaymentId().getValue(), source.getMethodOfPayment().getPaymentType(),
